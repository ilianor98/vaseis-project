...
def backup_database():
    ...
    backup_dest = request.args.get("file_destination")
    backup_name = request.args.get("file_name") + ".sql"

    mariadb_bin_dir = "C:/Program Files/Mariadb 11.1/bin"
    os.chdir(mariadb_bin_dir)

    command = f"mysqldump --host={db_host} --user={db_user} --password={db_password} --routines -B {db_name} > {backup_dest}"

    try:
        subprocess.run(command, shell=True, check=True)

        return send_file(backup_dest, as_attachment=True, download_name=backup_name)
